  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-72 sm:w-80 max-h-[80vh] overflow-y-auto" align="end" forceMount>
          <DropdownMenuLabel className="font-normal sticky top-0 bg-background z-10">
          ...
          <div className="p-2 sm:p-3">
            ...
            <div className="grid grid-cols-2 gap-2 mb-2 sm:mb-3">
              {/* Settled Groups */}
              <div className="flex items-center space-x-1.5 sm:space-x-2 p-1.5 sm:p-2 rounded-lg bg-green-500/10 border border-green-500/20">
                <CheckCircle className="h-3 w-3 sm:h-4 sm:w-4 text-green-600 flex-shrink-0" />
                <div>
                  <p className="text-xs font-medium text-green-700">Settled</p>
                  <p className="text-base sm:text-lg font-bold text-green-600">{settledGroups.length}</p>
                </div>
              </div>
              
              {/* Unsettled Groups */}
              <div className="flex items-center space-x-1.5 sm:space-x-2 p-1.5 sm:p-2 rounded-lg bg-red-500/10 border border-red-500/20">
                <Clock className="h-3 w-3 sm:h-4 sm:w-4 text-red-600 flex-shrink-0" />
                <div>
                  <p className="text-xs font-medium text-red-700">Pending</p>
                  <p className="text-base sm:text-lg font-bold text-red-600">{unsettledGroups.length}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Financial Summary */}
          <div className="p-2 sm:p-3">
            <h4 className="text-sm font-medium flex items-center mb-2 sm:mb-3">
            ...
            <div className="space-y-1.5 sm:space-y-2">
              {/* Amount You Owe */}
              <div className="flex items-center justify-between p-1.5 sm:p-2 rounded-lg bg-red-500/10">
                <span className="text-xs sm:text-sm text-red-700">You owe</span>
              ...
              {/* Amount Owed to You */}
              <div className="flex items-center justify-between p-1.5 sm:p-2 rounded-lg bg-green-500/10">
                <span className="text-xs sm:text-sm text-green-700">Owed to you</span>
              ...
              {/* Net Balance */}
              <div className="flex items-center justify-between p-1.5 sm:p-2 rounded-lg bg-muted">
                <span className="text-xs sm:text-sm font-medium">Net balance</span>
                <span className={`font-bold text-sm sm:text-base ${totalOwed - pendingDues >= 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          {userDues?.globalOptimalTransactions && userDues.globalOptimalTransactions.length > 0 && (
            <>
              ...
              <div className="p-2">
                ...
                <div className="space-y-1.5 max-h-24 sm:max-h-32 overflow-y-auto scrollbar-thin">
                  {userDues.globalOptimalTransactions.map((transaction, index) => (
                    <div key={index} className="p-1.5 sm:p-2 rounded-lg bg-muted/30 border">
          ...
          {userDues?.pendingGroups && userDues.pendingGroups.some(g => g.optimalTransactions?.length > 0) && (
            <>
              ...
              <div className="p-2">
                ...
                <div className="space-y-1.5 max-h-32 sm:max-h-40 overflow-y-auto scrollbar-thin">
                  {userDues.pendingGroups
                    .filter(group => group.optimalTransactions?.length > 0)
                    .map((group) => (
                    <div key={group.groupId} className="p-1.5 sm:p-2 rounded-lg bg-muted/30 border">
          ...
          {groups.length > 0 && (
            <>
              <div className="p-2 sm:p-3">
                ...
                <div className="space-y-1 max-h-24 sm:max-h-32 overflow-y-auto scrollbar-thin">
                  {groups.slice(0, 3).map((group) => (
                    <div key={group.id} className="flex items-center justify-between p-1.5 sm:p-2 rounded hover:bg-muted/50">
                      <div className="flex items-center space-x-1.5 sm:space-x-2 min-w-0 flex-1">
                        <div className={`w-2 h-2 rounded-full flex-shrink-0 ${group.isSettled ? 'bg-green-500' : 'bg-red-500'}`} />
                        <span className="text-xs sm:text-sm truncate max-w-[80px] sm:max-w-[120px]">{group.name}</span>
                      </div>
